...
class JobShopSchedulingModel:
    ...
    def define_cqm_variables(self) -> None:
        ...
        for job in self.model_data.jobs:
            for resource in self.model_data.resources:
                ...
                self._shift[(job, resource)] = lb  # Store the shift
                self._x[(job, resource)] = Integer(
                    "x{}_{}".format(job, resource), lower_bound=0, upper_bound=ub - lb
                )
        ...
        for job in self.model_data.jobs:  # job
            for prev_task, curr_task in zip(
                ...
                self.cqm.add_constraint(
                    (self._x[(job, machine_curr)] + self._shift[(job, machine_curr)]) -
                    (self._x[(job, machine_prev)] + self._shift[(job, machine_prev)]) >=
                    prev_task.duration,
            ...
            for k in self.model_data.jobs:
                if j < k:
                    for i in self.model_data.resources:
                        ...
                        self.cqm.add_constraint(
                            (self._x[(j, i)] + self._shift[(j, i)]) -
                            (self._x[(k, i)] + self._shift[(k, i)]) -
                            task_k.duration + self._y[(j, k, i)] * V >= 0,
                        ...
                        self.cqm.add_constraint(
                            (self._x[(k, i)] + self._shift[(k, i)]) -
                            (self._x[(j, i)] + self._shift[(j, i)]) -
                            task_j.duration + (1 - self._y[(j, k, i)]) * V >= 0,
    ...
    def add_makespan_constraint(self) -> None:
        ...
        for job in self.model_data.jobs:
            ...
            self.cqm.add_constraint(
                self._makespan_var -
                (self._x[(job, last_machine)] + self._shift[(job, last_machine)]) >=
                last_job_task.duration,
                label="makespan_ctr{}".format(job),
            )

    def call_cqm_solver(self, time_limit: int, profile: str) -> None:
        print(self.cqm)
        model = dynex.CQM(self.cqm)
        sampler = dynex.DynexSampler(model, mainnet=True, description='Quantum FSS', bnb=False)
        sampleset = sampler.sample(num_reads=256, annealing_time = 100, is_cluster=True)
        cqm_sample = sampleset.first.sample
        self._best_sample_cqm = cqm_sample

        self.solution = {
            (j, i): (
                self.model_data.get_resource_job_tasks(job=j, resource=i),
                self._best_sample_cqm[self._x[(j, i)].variables[0]] + self._shift[(j, i)],
