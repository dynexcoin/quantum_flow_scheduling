...
class GreedyJobShop:
    ...
    def solve(self, skip_probability: float = 0.1, seed: int = 42) -> dict:
        ...
        def resource_gap_finder(resource_schedule: list, min_start: int, gap_duration: int) -> dict:
            ...
            for i in range(len(resource_schedule) - 1):
                if (
                    resource_schedule[i + 1]["start"] > min_start
                    and resource_schedule[i + 1]["start"]
                    - max(min_start, resource_schedule[i]["finish"])
                    >= gap_duration
                ):
        ...
        remaining_task_times = {
            job: self.model_data.get_total_job_time(job) for job in self.model_data.jobs
        }
        ...
        while sum(not_yet_finished) > 0:
            ...
            if len(resource_schedules[resource]) == 0:
                ...
                min_resource_time, resource_pos = resource_gap_finder(
                    resource_schedules[resource], min_job_time, task.duration
                )

            start_time = min_resource_time
            finish_time = task.duration + start_time
            resource_schedules[resource].insert(
                resource_pos, {"start": start_time, "finish": finish_time, "task": task}
            )
